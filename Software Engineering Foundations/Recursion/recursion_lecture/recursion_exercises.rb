...
# this is similar to the fibonacci sequenece recursive exercise 
def lucas_number(n)
    return 2 if n == 0
    return 1 if n == 1
...
def sum_array(array)
    ...
    # if my base case has to do with ana array being trivially small
    # we need to make sure that our recursive step shrinks the array

    # we started with an array of length n, but on our 
    # recursive call, we need to pass in an array of n-1 length 
...
def reverse_string(str)
    ...
    # we need to recursively make the string smaller 
    # use the same method as the previous exercise 
...
def flatten(data)
    # if our argument is not an array, put it in 
    # an array to make it a 1D array
    return [data] if !data.is_a?(Array)

    # otherwise, the arg is an array
    flattened = []
    data.each do |ele|
        # we take the flattened data and add it to the flattened parent array
        flattened += flatten(ele) # flatten(ele) should give you an array back
    end 
